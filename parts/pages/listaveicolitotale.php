<?php
  ...
  if (isset($dati['ricerca'])) {
    ...
    if ($dati['who'] == 'admin') {
      $quer = "SELECT `id`, `id_piazzale`, `tipo_veicolo`, `sede`, `targa`, `telaio`, `marca`, `modello`, `colore`, `data_arrivo`,  `km`,   `data_uscita`,  `note`, `flotta`  FROM `macchina` where  (
      ...
      $totale_lavorazioni = tellMeTotaleAuto_();
    } else {
      $sede = addslashes($dati['sede']);
      $quer = "SELECT `id`, `id_piazzale`, `tipo_veicolo`, `sede`, `targa`, `telaio`, `marca`, `modello`, `colore`, `data_arrivo`,  `km`,   `data_uscita`,  `note`, `flotta`  FROM `macchina` WHERE `sede` = '$sede' and  (
        lower(`colore`) like '%$filter%' or
        lower(`data_arrivo`) like '%$filter%' or
        lower(`km`) like '%$filter%' or
        lower(`data_uscita`) like '%$filter%' or
        ...
        lower(`marca`) like '%$filter%' or
        lower(`modello`) like '%$filter%') LIMIT 1000";
      $totale_lavorazioni = tellMeTotaleAuto_($dati['sede']);
    }
  } else {
    ...
    if ($dati['who'] == 'admin') {
      $quer = "SELECT `id`, `id_piazzale`, `tipo_veicolo`, `sede`, `targa`, `telaio`, `marca`, `modello`, `colore`, `data_arrivo`,  `km`,   `data_uscita`,  `note`, `flotta`  FROM `macchina`   $limit";
      $totale_lavorazioni = tellMeTotaleAuto_();
    } else {
      $sede = addslashes($dati['sede']);
      $quer = "SELECT `id`, `id_piazzale`, `tipo_veicolo`, `sede`, `targa`, `telaio`, `marca`, `modello`, `colore`, `data_arrivo`,  `km`,   `data_uscita`,  `note`, `flotta`  FROM `macchina`  WHERE   `sede` = '$sede'  $limit";
      $totale_lavorazioni = tellMeTotaleAuto_($dati['sede']);
      // $totale_bolle = 0;
    }
  }
