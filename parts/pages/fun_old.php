...
function tellMeMacchineSedeLike($sede, $like){
  ...
  $db = new dbObj();
  $connString =  $db->getConnstring();
  $cec =  mysqli_query($connString, "SELECT * FROM `macchina` WHERE `sede` = '$sede' AND `flotta` LIKE '$like' AND data_arrivo > '$anno-12-31' ORDER BY `macchina`.`data_arrivo` ASC") or die("database error:". mysqli_error($connString));
  $ris = mysqli_fetch_all($cec);
  return $ris;
}
...
function tellMeCarsLavorazioni(){
	$db = new dbObj();
  $connString =  $db->getConnstring();
