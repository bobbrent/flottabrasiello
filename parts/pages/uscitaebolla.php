<?php
  ...
  $veicoli = tellMeCarsCancellazioneBolla();
  ...
  <div class="row mb">
    <div class="content-panel">
      <div class="col-lg-12">
            ...
            <select id="veicoli" >
              ...
              //  [flotta] => ALD
                foreach($veicoli as $v){
                  echo "<option value='".$v['id'].";".$v['bolla_uscita']."'>SEDE: ".$v['sede']." | ".$v['tipo_veicolo']." | ".$v['marca']." ".$v['modello']." TA.: ".$v['targa']." TE.: ".$v['telaio']." D.ING.: ".formattaDatano($v['data_arrivo'])." D.USC.: ".formattaDatano($v['data_uscita'])." BOLLA: ".$v['bolla_uscita']."</option>";
                }
              ?>
            </select>
          </div>
        ...
        <div class="col-lg-6">
          <button class="pulsante" onclick="cancella();" id="cancella">Elimina Data di Uscita</button>
        </div>
        <br><br><br><br>
      </div>
    </div>
...
<script type="text/javascript">
  ...
  function cancella(){
    var variabili = $('#veicoli').val().split(";");
    var co = $('#veicoli')[0][$('#veicoli')[0].selectedIndex].innerText;
    var r = confirm("Eliminare data di uscita e bolla di " + co + " ?");
		if (r) {
      dAzNidsV2('cancellauscitaebolla', variabili[0], variabili[1], 'veicolo');
    }
  }
