...
$ubiquity = "`tipo_veicolo` = 'NUOVO' and `data_uscita` IS NULL";
$pagina = 'listaveicoliinsedenuovo';
if (isset($dati['ricerca'])) {
  ...
  if ($dati['who'] == 'admin') {
    ...
    $totale_lavorazioni = tellMeTotaleAutoNuove();
  } else {
    ...
    $quer = "SELECT `id`, `id_piazzale`, `tipo_veicolo`, `sede`, `targa`, `telaio`, `marca`, `modello`, `colore`, `data_arrivo`,  `km`,   `data_uscita`,  `note`, `flotta`  FROM `macchina` WHERE `sede` = '$sede' and $ubiquity and  (
      ...
      lower(`marca`) like '%$filter%' or
      lower(`modello`) like '%$filter%') LIMIT 1000";
    $totale_lavorazioni = tellMeTotaleAutoNuove($dati['sede']);
  }
} else {
  ...
  if ($dati['who'] == 'admin') {
    $quer = "SELECT `id`, `id_piazzale`, `tipo_veicolo`, `sede`, `targa`, `telaio`, `marca`, `modello`, `colore`, `data_arrivo`,  `km`,   `data_uscita`,  `note`, `flotta`  FROM `macchina` WHERE `data_uscita` IS NULL and `tipo_veicolo` = 'NUOVO' $limit";
    $totale_lavorazioni = tellMeTotaleAutoNuove();
  } else {
    $sede = addslashes($dati['sede']);
    $quer = "SELECT `id`, `id_piazzale`, `tipo_veicolo`, `sede`, `targa`, `telaio`, `marca`, `modello`, `colore`, `data_arrivo`,  `km`,   `data_uscita`,  `note`, `flotta`  FROM `macchina`  WHERE `data_uscita` IS NULL and `sede` = '$sede' and `tipo_veicolo` = 'NUOVO' $limit";
    $totale_lavorazioni = tellMeTotaleAutoNuove($dati['sede']);
    // $totale_bolle = 0;
  }
}
              ...
              // print_r($veicoli);
                if($dati['who'] == 'admin'){
                  foreach ( $veicoli as $v ){
                    if($v['tipo_veicolo'] == 'NUOVO'){
                      $stringa = "<tr><td>%s</td><td>%s</td><td><input type='text' class='editabili' name='targa' id='%s' value='%s'></td><td><input type='text' class='editabili' name='telaio' id='%s' value='%s'></td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td><input type='text' class='editabili' name='km' id='%s' value='%s'></td><td>%s</td><td><input type='text' class='editabili' name='note' id='%s' value='%s'></td></tr>";
                      printf($stringa, $v['flotta'],tellMePiazzale($v['id_piazzale']),$v['id'],$v['targa'],$v['id'],$v['telaio'],$v['marca'],$v['modello'],$v['colore'],formattaDataLogin($v['data_arrivo']),$v['id'],$v['km'],formattaDataLogin($v['data_uscita']),$v['id'],$v['note']);
                    }
                  }
                }else{
                  foreach ( $veicoli as $v ){
                    // print_r($v);
                    if($v['sede'] == $dati['sede'] && $v['tipo_veicolo'] == 'NUOVO'){
                      $stringa = "<tr><td>%s</td><td>%s</td><td><input type='text' class='editabili' name='targa' id='%s' value='%s'></td><td><input type='text' class='editabili' name='telaio' id='%s' value='%s'></td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>";
                      printf($stringa, $v['flotta'],tellMePiazzale($v['id_piazzale']),$v['id'],$v['targa'],$v['id'],$v['telaio'],$v['marca'],$v['modello'],$v['colore'],formattaDataLogin($v['data_arrivo']),$v['km'],formattaDataLogin($v['data_uscita']),$v['note']);  
                    }
                  }
                }
              ?>
