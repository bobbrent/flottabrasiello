<?php
  include 'fun.php';
  // $veicoli = tellMeCars();
  $ubiquity = "`tipo_veicolo` = 'NUOVO' ";
  $pagina = 'listaveicolinuovo';
  if (isset($dati['ricerca'])) {
    ...
    if ($dati['who'] == 'admin') {
      ...
      $totale_lavorazioni = tellMeTotaleAutonuovo();
    } else {
      ...
      $quer = "SELECT `id`, `id_piazzale`, `tipo_veicolo`, `sede`, `targa`, `telaio`, `marca`, `modello`, `colore`, `data_arrivo`,  `km`,   `data_uscita`,  `note`, `flotta`  FROM `macchina` WHERE `sede` = '$sede' and $ubiquity and  (
        ...
        lower(`marca`) like '%$filter%' or
        lower(`modello`) like '%$filter%') LIMIT 1000";
      $totale_lavorazioni = tellMeTotaleAutonuovo($dati['sede']);
    }
  } else {
                ...
                if($dati['who'] == 'admin'){
                  foreach ( $veicoli as $v ){
                    if($v['tipo_veicolo'] == 'NUOVO'){
                      ...
                      printf($stringa, $v['flotta'],tellMePiazzale($v['id_piazzale']),$v['targa'],$v['telaio'],$v['marca'],$v['modello'],$v['colore'],formattaDataLogin($v['data_arrivo']),$v['km'],formattaDataLogin($v['data_uscita']),$v['note']);
                    }
                  }
                }else{
                  foreach ( $veicoli as $v ){
